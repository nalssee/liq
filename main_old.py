...
with connect('db.db') as c:
    ...
    c.load(fnguide1('code.xlsx', 'code'), 'indcode')
...
with connect('db.db') as c:
    c.create("select ta, ppe, sales, rnd, id, year + 1 as year1 from comp_kor", "temp")

    c.join(
        ["comp_kor", "*", "id, year"],
        ["temp", "ta as ta1, ppe as ppe1, sales as sales1, rnd as rnd1", "id, year1"],
        ['size', 'size', "id, year"],
        name='comp'
    )
    # inflation 을 붙이고
...
with connect('db.db') as c:
    ...
    print(c.df('comp', cols='rating').describe())
